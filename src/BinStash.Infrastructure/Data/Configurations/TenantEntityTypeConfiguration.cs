...
public class TenantEntityTypeConfiguration : IEntityTypeConfiguration<Tenant>
{
    public void Configure(EntityTypeBuilder<Tenant> builder)
    {
        ...
        // Optional: the setup-created default tenant has no creator (it is created before any user exists).
        builder.Property(t => t.CreatedByUserId).IsRequired(false);
        builder.Property(t => t.Status).IsRequired().HasDefaultValue(TenantStatus.Active);

        builder.HasIndex(t => t.Slug).IsUnique();

        builder.HasOne<BinStashUser>().WithMany().HasForeignKey(t => t.CreatedByUserId).IsRequired(false);
    }
}