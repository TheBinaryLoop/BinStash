...
5. **Legacy migration** — SVN import subsystem enables bulk migration of tagged SVN releases into BinStash with resumable state tracking.

## How it works (user perspective)

1. Operator sets up a server with PostgreSQL and a local chunk store.
2. Developer/CI runner uses `BinStash.Cli release add` pointing at a build output directory (or ZIP archive).
...
- Analyze commands (`analyze release`, `analyze rdef`) provide insight into release composition and storage efficiency.
...
- Efficient binary format (V4 `.rdef` is 30.4% smaller than V2) minimizes network and storage overhead for release metadata.
